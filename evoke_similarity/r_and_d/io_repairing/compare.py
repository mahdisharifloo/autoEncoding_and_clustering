...
class Compare:
    def __init__(self,data_file='data.pkl'):
        ...
        self.product_id = list(self.data['product_id'])
        self.counter = 0
        self.mse_all = []
        self.ssim_all = [] 
    ...
    def create_dataframe( self,mse_all ,ssim_all ):
        ...
        df_mse = pd.DataFrame(mse_all,columns=['image path','product_id','compare val'])
        df_ssim = pd.DataFrame(ssim_all,columns=['image path','product_id','compare val'])
