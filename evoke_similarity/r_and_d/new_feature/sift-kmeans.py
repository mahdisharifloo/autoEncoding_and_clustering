...
def build_arg_parser():
    ...
    parser.add_argument("--feature-map-file", dest='feature_map_file',parser.add_argument("--feature-map-file", dest='feature_map_file',help="Base file name to store the feature map"))
    return parser

# Loading the images from the input folder
def load_input_map(label,input_foder):
    combined_data = []
...
class FeatureExetractor(object):
...
if __name__=='__main__':
    ...
    input_map = []
    for cls in args.cls:
        assert len(cls) >= 2, "Format for classes is `<label> file`"label = cls[0],input_map += load_input_map(label, cls[1])
