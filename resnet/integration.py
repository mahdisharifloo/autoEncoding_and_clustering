...
def resnet50_feature_extractor(preprocess_image):
    # preprocessing for image input the vgg_server
    # preprocess_image = nn_image_preprocessing(image_bytes)
    processed_imgs = resnet50.preprocess_input(preprocess_image)
...
if __name__ == "__main__":
    # import resnet50 model for extracrating image features.
    mobilenet_extractor = resnet50.ResNet50(weights='imagenet', include_top=False,input_shape=(224, 224, 3))
    ...
    for i,file_name in enumerate(data_names):
        ...
        mobilenet_feature_arr = resnet50_feature_extractor(preprocess_image)
